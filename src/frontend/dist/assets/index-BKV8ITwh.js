...
const LocateFixed = createLucideIcon("locate-fixed", __iconNode$8);
/**
 * @license lucide-react v0.511.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
function scaleToScreen(realWaveHeight) {
  const minReal = 0.3;
  const maxReal = 6;
  const minScreen = 6;
  const maxScreen = 90;
  const clamped = Math.min(Math.max(realWaveHeight, minReal), maxReal);
  return minScreen + (clamped - minReal) / (maxReal - minReal) * (maxScreen - minScreen);
}
function lerp(a, b, t) {
  return a + (b - a) * t;
}
function updateWaves(waveHeight, waves) {
  ...
  waves[0].targetFrequency = 6e-3 + waveHeight / 6 * 6e-3;
...
function getY(x, wave) {
  ...
  return wave.sharp ? Math.sign(raw) * Math.abs(raw) ** 0.65 * wave.amplitude : raw * wave.amplitude;
}
function HeroWaveCanvas({
  ...
  const stateRef = reactExports.useRef({
    waves: [],
    currentColor: RATING_COLORS[rating],
    targetColor: RATING_COLORS[rating],
    tweenStart: 0,
    tweenDuration: 1200,
    whitecaps: [],
    windSpeed,
  ...
  reactExports.useEffect(() => {
    ...
    if (state.waves.length === 0) {
      const amplitude = scaleToScreen(waveHeight);
      const speed1 = 0.3 + waveHeight / 6 * 1.4;
      const freq = 6e-3 + waveHeight / 6 * 6e-3;
      state.waves = [
        {
          amplitude,
  ...
  reactExports.useEffect(() => {
    ...
    const animate = (now2) => {
      ...
      const tweenT = Math.min(
        (now2 - state.tweenStart) / state.tweenDuration,
        1
      );
      const cc = state.currentColor;
      const tc = state.targetColor;
      const r2 = Math.round(lerp(cc[0], tc[0], tweenT));
      ...
      for (const idx of drawOrder) {
        ...
        grad.addColorStop(0, `rgba(${r2},${g},${b},${wave.opacity})`);
        ...
        if (idx === 0 && state.windSpeed > 25) {
          for (let i = 1; i < points.length - 1; i++) {
            ...
            if (py < prevY && py < nextY && Math.random() < 3e-3) {
...
const DEFAULT_UNITS = { height: "ft", temp: "F", speed: "kts" };
function loadUnits() {
  try {
    const raw = localStorage.getItem(UNITS_KEY);
    if (!raw) return DEFAULT_UNITS;
    const parsed = JSON.parse(raw);
    if (typeof parsed.height === "string" && typeof parsed.temp === "string" && typeof parsed.speed === "string") {
      return parsed;
    }
...
function loadPresets() {
  try {
    ...
    for (let i = 0; i < 5; i++) {
      const s = parsed[i];
      if (s && typeof s.name === "string" && typeof s.country === "string" && typeof s.lat === "number" && typeof s.lng === "number" && typeof s.timezone === "string") {
        slots[i] = s;
      }
    }
    return slots;
...
function useCountUp(target, duration = 1e3, decimals = 1) {
  ...
  reactExports.useEffect(() => {
    ...
    const tick = (now2) => {
      const p = Math.min((now2 - start) / duration, 1);
      const eased = 1 - (1 - p) ** 3;
      setVal(eased * target);
...
function useLocalTime(timezone) {
  ...
  reactExports.useEffect(() => {
    ...
    const tick = () => setTime(fmt.format(/* @__PURE__ */ new Date()));
    tick();
...
async function fetchConditions(spot) {
  ...
  const { lat, lng } = spot;
  const tz = spot.timezone === "auto" ? "auto" : encodeURIComponent(spot.timezone);
  ...
  const nowLocalMs = Date.now() + utcOffsetSeconds * 1e3;
  ...
  const hourlyTimes = ((_a2 = marine.hourly) == null ? void 0 : _a2.time) ?? [];
  let hi = hourlyTimes.findIndex((t) => t === currentHourStr);
  if (hi < 0) {
    hi = hourlyTimes.reduce((best, t, idx) => {
      const diff = Math.abs(
        new Date(t).getTime() - (Date.now() + utcOffsetSeconds * 1e3)
      );
      const bestDiff = Math.abs(
        new Date(hourlyTimes[best] ?? t).getTime() - (Date.now() + utcOffsetSeconds * 1e3)
      );
      return diff < bestDiff ? idx : best;
    }, 0);
  }
  const wHourlyTimes = ((_b2 = weather.hourly) == null ? void 0 : _b2.time) ?? [];
  let wi = wHourlyTimes.findIndex((t) => t === currentHourStr);
  if (wi < 0) {
    wi = wHourlyTimes.reduce((best, t, idx) => {
      ...
      const bestDiff = Math.abs(
        new Date(wHourlyTimes[best] ?? t).getTime() - (Date.now() + utcOffsetSeconds * 1e3)
      );
      return diff < bestDiff ? idx : best;
    }, 0);
  }
  const waveHeight = ((_d2 = (_c2 = marine.hourly) == null ? void 0 : _c2.wave_height) == null ? void 0 : _d2[hi]) ?? 1.2;
  const waveDirection = ((_f2 = (_e2 = marine.hourly) == null ? void 0 : _e2.wave_direction) == null ? void 0 : _f2[hi]) ?? 270;
  const windSpeed = ((_g2 = weather.current_weather) == null ? void 0 : _g2.windspeed) ?? ((_i2 = (_h2 = weather.hourly) == null ? void 0 : _h2.windspeed_10m) == null ? void 0 : _i2[wi]) ?? 15;
  const windDirection = ((_j = weather.current_weather) == null ? void 0 : _j.winddirection) ?? ((_l = (_k = weather.hourly) == null ? void 0 : _k.winddirection_10m) == null ? void 0 : _l[wi]) ?? 270;
  const waterTemp = ((_n = (_m = weather.hourly) == null ? void 0 : _m.apparent_temperature) == null ? void 0 : _n[wi]) ?? 18;
  const airTemp = ((_p = (_o = weather.hourly) == null ? void 0 : _o.temperature_2m) == null ? void 0 : _p[wi]) ?? 20;
  const dailyWindSpeed = ((_q = weather.daily) == null ? void 0 : _q.windspeed_10m_max) ?? Array(7).fill(15);
  const dailyWaveMax = ((_r = marine.daily) == null ? void 0 : _r.wave_height_max) ?? [
    1,
    1.2,
    0.8,
    1.5,
  ...
  const dailyWaveDir = ((_s = marine.daily) == null ? void 0 : _s.wave_direction_dominant) ?? Array(7).fill(270);
  const dailyDates = ((_t = marine.daily) == null ? void 0 : _t.time) ?? [...Array(7).keys()].map((i) => {
    const d = /* @__PURE__ */ new Date();
    d.setDate(d.getDate() + i);
    return d.toISOString().slice(0, 10);
  });
  const allTideHeights = ((_u = marine.hourly) == null ? void 0 : _u.sea_level_height_msl) ?? [];
  const marineHourlyTimes = ((_v = marine.hourly) == null ? void 0 : _v.time) ?? [];
  ...
  const todayStartIdx = marineHourlyTimes.findIndex(
    (t) => t.startsWith(todayDateStr)
  );
  const rawTide = todayStartIdx >= 0 ? allTideHeights.slice(todayStartIdx, todayStartIdx + 24) : Array(24).fill(null);
  const rawTideFilled = rawTide.map((v, i, arr) => {
    if (v !== null && v !== void 0) return v;
    let lo = i - 1;
    let hi2 = i + 1;
    while (lo >= 0 && (arr[lo] === null || arr[lo] === void 0)) lo--;
    while (hi2 < arr.length && (arr[hi2] === null || arr[hi2] === void 0)) hi2++;
    const loVal = lo >= 0 ? arr[lo] : null;
    const hiVal = hi2 < arr.length ? arr[hi2] : null;
    if (loVal !== null && hiVal !== null)
      return (loVal + hiVal) / 2;
    if (loVal !== null) return loVal;
    if (hiVal !== null) return hiVal;
    return 0;
  });
  const allValidHeights = allTideHeights.filter(
    (v) => typeof v === "number" && Number.isFinite(v)
  );
  const chartDatumOffset = allValidHeights.length > 0 ? Math.min(...allValidHeights) : 0;
  const tideHeights = rawTideFilled.map((v) => v - chartDatumOffset);
  return {
    waveHeight,
    waveDirection,
    windSpeed,
      ...
      "div",
      {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "dialog",
          {
            ...
            style: {
              background: "linear-gradient(145deg, rgba(13,30,52,0.98) 0%, rgba(2,13,24,0.99) 100%)",
            ...
            children: [
                ...
                "div",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("h2", { className: "font-display font-bold text-white text-lg", children: [
                        "Assign Preset ",
                        slotIndex + 1
                      ] }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "p",
                        {
                          ...
                          children: "Search any surf spot worldwide"
                        }
                      )
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-6 pt-5 pb-3", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
              ...
              currentSlot && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-6 pb-3", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  type: "button",
                  onClick: () => {
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
                  ...
                  children: [
                    results.length === 0 && query.trim() && !searching && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      "div",
                      {
                    ...
                    results.length === 0 && !query.trim() && /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        className: "px-6 py-6 text-center font-body text-xs",
                        style: { color: "var(--color-seafoam)", opacity: 0.4 },
                        children: "Start typing to search surf spots worldwide"
                      }
                    ),
                    results.map((result, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      "button",
                      {
                        type: "button",
                        onClick: () => {
                        ...
                        onMouseEnter: (e) => {
                          e.currentTarget.style.background = "rgba(13,79,110,0.3)";
                        },
                        onMouseLeave: (e) => {
                        ...
                        children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx(
                            "div",
                            {
                              ...
                              children: /* @__PURE__ */ jsxRuntimeExports.jsx(Waves, { size: 12, style: { color: "var(--color-electric)" } })
                            }
                          ),
                          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-body font-semibold text-white text-sm truncate", children: result.name }),
                            /* @__PURE__ */ jsxRuntimeExports.jsxs(
                              "div",
                              {
...
function PresetBar({
  ...
  const [modalSlot, setModalSlot] = reactExports.useState(null);
  const [draggingIndex, setDraggingIndex] = reactExports.useState(null);
  ...
  const handleSelect = (spot, slotIndex) => {
    const next = [...presets];
    next[slotIndex] = spot;
    setPresets(next);
    loadSpot(spot);
  ...
  const handleDragOver = (e, index2) => {
    ...
    if (index2 !== dragOverIndex) setDragOverIndex(index2);
  };
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("fieldset", { className: "border-0 p-0 m-0", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("legend", { className: "sr-only", children: "Preset surf spots" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-wrap gap-2 md:gap-3", children: presets.map((spot, i) => {
        const isSelected = spot !== null && (selectedSpot == null ? void 0 : selectedSpot.name) === spot.name;
            ...
            "div",
            {
              ...
              children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  style: {
                    background: isDragTarget ? "rgba(0,180,216,0.12)" : "rgba(13,79,110,0.15)",
                    border: isDragTarget ? "1px dashed var(--color-electric)" : "1px dashed rgba(13,79,110,0.5)",
                    color: isDragTarget ? "var(--color-electric)" : "rgba(168,216,200,0.45)"
                  },
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  style: {
                    background: isSelected ? "var(--color-electric)" : "rgba(13,79,110,0.35)",
                    border: `1px solid ${isSelected ? "var(--color-electric)" : "rgba(13,79,110,0.7)"}`,
                    color: isSelected ? "var(--color-bg)" : "rgba(255,255,255,0.88)",
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "truncate max-w-[120px]", children: spot.name }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "span",
                      {
                        className: "text-xs opacity-60 truncate max-w-[80px]",
                        style: {
                          color: isSelected ? "rgba(2,13,24,0.7)" : "var(--color-seafoam)"
                        },
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  ...
                  style: {
                    background: isSelected ? "rgba(2,13,24,0.25)" : "rgba(13,79,110,0.5)",
                    color: isSelected ? "rgba(2,13,24,0.8)" : "var(--color-seafoam)"
                  },
    ...
    modalSlot !== null && /* @__PURE__ */ jsxRuntimeExports.jsx(
      SearchModal,
      {
        slotIndex: modalSlot,
...
function StatCard({
  ...
  const countedStr = useCountUp(value, 1e3, decimals);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      style: {
        background: "linear-gradient(135deg, rgba(13,79,110,0.45) 0%, rgba(2,13,24,0.7) 100%)",
        border: "1px solid rgba(13,79,110,0.6)",
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "var(--color-electric)" }, className: "opacity-70", children: icon }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-baseline gap-2 leading-none flex-wrap min-w-0", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "svg",
              {
                ...
                children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "10", cy: "10", r: "1.5", fill: "rgba(168,216,200,0.6)" })
                ]
              }
            )
          }
...
function ForecastCard({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      style: {
        animationDelay: `${index2 * 50}ms`,
        background: "rgba(13,79,110,0.25)",
        border: "1px solid rgba(13,79,110,0.5)"
      },
      "data-ocid": `forecast.item.${index2 + 1}`,
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "span",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-end justify-center", style: { height: 64 }, children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            style: {
              width: 12,
              height: barH,
              background: "linear-gradient(to top, var(--color-electric), var(--color-seafoam))",
              borderRadius: 4,
              transition: "height 0.5s ease"
            }
          }
        ) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "font-display font-bold text-white text-base", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm", style: { color: "var(--color-electric)" }, children: compassDir }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            style: {
...
function TideChart({
  ...
  const toSVGY = (v) => H - (v - yLo) / (yHi - yLo) * (H * 0.8) - H * 0.1;
  let pathD = "";
  ...
  const localMs = nowUtcMs + utcOffsetSeconds * 1e3;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-6 pt-5 pb-2", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "h3",
          {
            className: "font-body text-xs tracking-widest uppercase font-semibold",
            style: { color: "var(--color-seafoam)", opacity: 0.8 },
            children: "Tide Chart — Today"
          }
        ) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-4 pb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "svg",
          {
            ...
            style: { height: 200 },
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("title", { children: "Today's Tide Chart" }),
              gridFracs.map((frac) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                "line",
                {
                  x1: 0,
                  y1: H * frac,
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("defs", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("linearGradient", { id: "tideGradient", x1: "0", y1: "0", x2: "0", y2: "1", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("stop", { offset: "0%", stopColor: "#00b4d8", stopOpacity: 0.6 }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("stop", { offset: "100%", stopColor: "#00b4d8", stopOpacity: 0 })
              ] }) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "line",
                {
              ...
              timeLabels.map((label, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                "text",
                {
                  x: i / 4 * W,
                  y: H + 22,
              ...
              ].map(({ value, y }) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "text",
                {
                  x: W - 4,
                  y,
...
function LoadingSkeleton() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-4", children: [1, 2, 3, 4].map((i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
...
function SpotHeader({ spot }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-baseline gap-3 flex-wrap", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "font-display font-bold text-2xl text-white", children: spot.name }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "span",
      {
...
function App() {
  const [presets, setPresetsState] = reactExports.useState(() => loadPresets());
  const [selectedSpot, setSelectedSpot] = reactExports.useState(null);
  const [data, setData] = reactExports.useState(null);
  const [loading, setLoading] = reactExports.useState(false);
  const [error, setError] = reactExports.useState(null);
  const [units, setUnitsState] = reactExports.useState(loadUnits);
  const [settingsOpen, setSettingsOpen] = reactExports.useState(false);
  const setUnits = reactExports.useCallback((next) => {
    setUnitsState(next);
    saveUnits(next);
  }, []);
  const [tileOrder, setTileOrderState] = reactExports.useState(
    () => loadTileOrder()
  );
  const dragTileRef = reactExports.useRef(null);
  const setTileOrder = reactExports.useCallback((next) => {
  ...
  const setPresets = reactExports.useCallback((next) => {
    setPresetsState(next);
  }, []);
  const loadSpot = reactExports.useCallback(async (spot) => {
  ...
  const handleUseMyLocation = reactExports.useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          const name = ((_a2 = geoData.address) == null ? void 0 : _a2.city) || ((_b2 = geoData.address) == null ? void 0 : _b2.town) || ((_c2 = geoData.address) == null ? void 0 : _c2.village) || geoData.name || "My Location";
          const country = ((_d2 = geoData.address) == null ? void 0 : _d2.country) ?? "";
          const spot = {
            name,
            country,
            lat,
  ...
  const displayHeight = (m) => units.height === "ft" ? metresToFeet(m) : m;
  const heightLabel = units.height === "ft" ? "ft" : "m";
  const displayTemp = (c) => units.temp === "F" ? celsiusToFahrenheit(c) : c;
  const tempLabel = units.temp === "F" ? "°F" : "°C";
  const displaySpeed = (k) => units.speed === "kts" ? kmhToKnots(k) : units.speed === "mph" ? kmhToMph(k) : k;
  const speedLabel = units.speed;
  const overallRating = data ? waveStatus(data.waveHeight) === "good" && windStatus(data.windSpeed) === "good" ? "good" : waveStatus(data.waveHeight) === "poor" || windStatus(data.windSpeed) === "poor" ? "poor" : "average" : "average";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          HeroWaveCanvas,
          {
            waveHeight: (data == null ? void 0 : data.waveHeight) ?? 1.2,
            windSpeed: (data == null ? void 0 : data.windSpeed) ?? 10,
            rating: overallRating
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative", style: { zIndex: 1 }, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("header", { className: "px-6 md:px-12 pt-8 pb-2", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-7xl mx-auto flex items-center justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(Sheet, { open: settingsOpen, onOpenChange: setSettingsOpen, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
            SheetContent,
            {
              side: "right",
              className: "w-80 border-0 p-0",
              style: {
                background: "linear-gradient(170deg, rgba(13,30,52,0.98) 0%, rgba(2,13,24,0.99) 100%)",
              ...
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  SheetHeader,
                  {
                    ...
                    children: /* @__PURE__ */ jsxRuntimeExports.jsxs(SheetTitle, { className: "font-display font-bold text-white flex items-center gap-2", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        Settings,
                        {
                          size: 16,
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-6 py-6 space-y-8", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "p",
                      {
                        className: "font-body text-xs tracking-widest uppercase font-semibold",
                        style: { color: "var(--color-seafoam)", opacity: 0.6 },
                        children: "Wave Height"
                      }
                    ),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-2", children: ["ft", "m"].map((opt) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "button",
                      {
                        type: "button",
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-2", children: ["F", "C"].map((opt) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "button",
                      {
                        type: "button",
                        onClick: () => setUnits({ ...units, temp: opt }),
                        className: "flex-1 rounded-xl py-3 font-body text-sm font-semibold transition-all duration-200",
                        style: units.temp === opt ? {
                          background: "var(--color-electric)",
                          color: "var(--color-bg)",
                          border: "1px solid var(--color-electric)"
                        } : {
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-2", children: ["kts", "mph", "km/h"].map((opt) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "button",
                      {
                        type: "button",
                        onClick: () => setUnits({ ...units, speed: opt }),
                        className: "flex-1 rounded-xl py-3 font-body text-sm font-semibold transition-all duration-200",
                        style: units.speed === opt ? {
                          background: "var(--color-electric)",
                          color: "var(--color-bg)",
                          border: "1px solid var(--color-electric)"
                        } : {
                        ...
                        children: opt === "kts" ? "Knots" : opt === "mph" ? "MPH" : "KM/H"
                      },
                      opt
                    )) })
                  ] })
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "px-6 md:px-12 pt-10 pb-8", "data-ocid": "hero.section", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-7xl mx-auto", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "h1",
              {
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-5 flex flex-col items-start gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  type: "button",
                  ...
                  style: {
                    background: locating ? "rgba(0,180,216,0.1)" : "rgba(0,180,216,0.08)",
                    border: "1px solid rgba(0,180,216,0.35)",
                    color: "var(--color-electric)"
                  },
                  "aria-label": "Use my current location to detect nearest surf conditions",
                  children: [
                    locating ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: locating ? "Detecting location…" : "Use my location" })
                  ]
                }
              ),
            ...
            !selectedSpot && !locating && /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "p",
              {
                ...
                children: [
                  ...
                  " ",
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "var(--color-electric)" }, children: "+" }),
                  " to add a new spot"
                ]
              }
            )
          ] }) }),
          selectedSpot && /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "px-6 md:px-12 pb-16", "data-ocid": "dashboard.section", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-7xl mx-auto space-y-6", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(SpotHeader, { spot: selectedSpot }),
            loading && /* @__PURE__ */ jsxRuntimeExports.jsx(LoadingSkeleton, {}),
            ...
            data && !loading && /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { "data-ocid": "conditions.section", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "h3",
                {
                  className: "font-body text-xs tracking-widest uppercase font-semibold mb-4",
                  style: { color: "var(--color-seafoam)", opacity: 0.7 },
                  children: "Today's Conditions"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-4", children: tileOrder.map((tileId, idx) => {
                ...
                const tileContent = (() => {
                  switch (tileId) {
                    case "wave":
                      return /* @__PURE__ */ jsxRuntimeExports.jsx(
                        StatCard,
                        {
                          ...
                          icon: /* @__PURE__ */ jsxRuntimeExports.jsx(Waves, { size: 14 }),
                          decimals: 1
                        }
                      );
                    case "wind":
                    ...
                    case "direction":
                      return /* @__PURE__ */ jsxRuntimeExports.jsx(CompassCard, { degrees: data.windDirection });
                    case "waterTemp":
                      return /* @__PURE__ */ jsxRuntimeExports.jsx(
                        StatCard,
                        {
                          ...
                          icon: /* @__PURE__ */ jsxRuntimeExports.jsx(Thermometer, { size: 14 }),
                          decimals: 0
                        }
                      );
                    case "airTemp":
                    ...
                    case "tide":
                      return /* @__PURE__ */ jsxRuntimeExports.jsx(
                        TideChart,
                        {
                          tideHeights: (data == null ? void 0 : data.tideHeights) ?? [],
                          utcOffsetSeconds: (data == null ? void 0 : data.utcOffsetSeconds) ?? 0,
                          displayHeight,
                          heightLabel
                        }
                      );
                      ...
                      return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                        "div",
                        {
                          ...
                          children: [
                            ...
                            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-4 pb-4 overflow-x-auto no-scrollbar", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                              "div",
                              {
                                ...
                                children: (() => {
                                  const nowLocalMs = Date.now() + (data.utcOffsetSeconds ?? 0) * 1e3;
                                  const nowLocal = new Date(nowLocalMs);
                                  ...
                                  return data.dailyDates.slice(todayIdx, todayIdx + 7).map((date, j) => {
                                    const i = todayIdx + j;
                                    return /* @__PURE__ */ jsxRuntimeExports.jsx(
                                      ForecastCard,
                                      {
                ...
                return /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "div",
                  {
                    ...
                    onDrop: () => {
                      if (dragTileRef.current === null || dragTileRef.current === idx)
                        return;
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "p",
                {
                  ...
                  children: "Drag tiles to reorder"
                }
              )
            ] }) })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "footer",
            {
              ...
              children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-7xl mx-auto flex items-center justify-between flex-wrap gap-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "p",
                  {
                    className: "font-body text-xs",
                    style: { color: "var(--color-seafoam)", opacity: 0.4 },
                    children: "Data from Open-Meteo Marine API"
                  }
                ),
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "p",
